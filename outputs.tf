...
output "public_key_filename" {
  ...
  value = length(tls_private_key.default.*.public_key_openssh) > 0 ? local.public_key_filename : local.public_key_filename
}
...
output "private_key_filename" {
  ...
  # Prevent releasing filename to downstream consumers until file exists (aka not during plan):
  value = length(tls_private_key.default.*.public_key_openssh) > 0 ? local.private_key_filename : local.private_key_filename
}
